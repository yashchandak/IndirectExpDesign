...
from datasets.data_TA import TripAdvisor
from bootstrap.bootstrap_CIV import BootstrapMSE
...
# config.update("jax_debug_nans", True) #TODO
...
class Parser(object):
    def __init__(self):
        ...
        parser.add_argument("--experiment", default='bootCIV', help="Name of the experiment")
    ...
    def Experiment_setup_args(self, parser):
        parser.add_argument("--n_IV", default=int(10), help="number of IVs", type=int)
        parser.add_argument("--conf_strength", default=1.0, help="Strength of confounder", type=float)
        parser.add_argument("--total_samples", default=int(5000), help="number of samples per batch", type=int)
        parser.add_argument("--samples_first_batch", default=int(1000), help="number of samples per batch", type=int)
    ...
    def Env_n_Agent_args(self, parser):
        ...
        parser.add_argument("--env_name", default='semi-synthetic', help="Environment to run the code",
                           choices=['synthetic', 'semi-synthetic'] )  

    def Main_args(self, parser):
        parser.add_argument("--sampler_epochs", default=int(1e1), help="Number of training epochs for sampler", type=int)
        ...
        parser.add_argument("--inner_epochs", default=int(5e2), help="Number of inner training epochs for est/nuisance", type=int)
        parser.add_argument("--lr_start", default=1e-2, help="Starting learning rate", type=float)
        parser.add_argument("--lr_end", default=1e-3, help="Ending learning rate", type=float)
        parser.add_argument("--reset_inner_param", default=False, help="reset inner params between each bootstrap?", type=self.str2bool)
        
        parser.add_argument("--hess_inv", default='CG', help=" [CG or Neumann] ")
        # parser.add_argument("--neumann_alpha", default=5e-1, help="1e-0 Scaling factor of Hessian for Neumann series expansion", type=float)
        # parser.add_argument("--hinv_iter", default=5, help="Number of iteration for H_inv approx", type=int)
        parser.add_argument("--subsampling_power", default=0.6, help=" 0.75 Bootstrap subsample size", type=float)
        parser.add_argument("--boot_sample_size", default=10, help="Number of bootstrap dataset replicas", type=int)
        
        parser.add_argument("--inverse_reg", default=0, help="Coeff for compliance regularization", type=float) #UNUSED
...
if __name__ == "__main__":
        ...
        # For running the code on laptop. Manual parallelization.
            TRIALS = 5
            _ = Parallel(n_jobs=min(TRIALS, 5))(delayed(main)(args, inc=idx) for idx in range(TRIALS))
